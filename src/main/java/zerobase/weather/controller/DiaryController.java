...
public class DiaryController {
    ...
    @ApiOperation(value = "일기 텍스트와 날씨를 이용해서 DB에 일기를 저장합니다.",
            notes = "날짜와 내용을 입력하면 해당 날짜의 날씨와 함께 일기를 저장할 수 있습니다.")
    @PostMapping("/create/diary")
    void createDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                     @ApiParam(value = "일기를 작성할 날짜", example = "2020-02-02") LocalDate date,
    ...
    @ApiOperation("선택한 날짜의 첫 번째 일기 text 내용을 수정합니다.")
    @PutMapping("/update/diary")
    void updateDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                     @ApiParam(value = "일기를 수정할 날짜", example = "2020-02-02") LocalDate date,
                     @RequestBody @ApiParam(value = "수정할 내용") String text){
    ...
    void deleteDiary(@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
                     @ApiParam(value = "일기를 삭제할 날짜", example = "2020-02-02") LocalDate date){
